...
class SupabasePsychometricRepository implements PsychometricRepository {
  ...
  Future<void> syncToCloud(PsychometricProfile profile) async {
    ...
    if (userId == null) {
      if (kDebugMode) debugPrint('SupabasePsychometricRepository: No user logged in, skipping cloud sync.');
      return;
    }
    ...
    try {
      final data = {
        'user_id': userId,
        'anti_identity_label': profile.antiIdentityLabel ?? '', // Handle nullable
      ...
      if (kDebugMode) debugPrint('SupabasePsychometricRepository: Synced profile to cloud for user $userId');
      
    } catch (e) {
      if (kDebugMode) debugPrint('SupabasePsychometricRepository: Error syncing to cloud: $e');
      // We don't rethrow because cloud sync failure shouldn't crash the app app flow 
  ...
  Future<PsychometricProfile?> getProfile() async {
    ...
    try {
      ...
      // Parse response into PsychometricProfile
      // We need a factory method in PsychometricProfile or mapping logic here.
      // Since PsychometricProfile.fromSupabase doesn't exist yet (based on prompt),
      // mapping manually or assuming we will add it.
      // Given I cannot modify domain entity interface easily without verifying, 
      // I will implement a safe mapper helper here.
      
      return _mapSupabaseDataToProfile(response);

    } catch (e) {
      if (kDebugMode) debugPrint('SupabasePsychometricRepository: Error fetching from cloud: $e');
      return null;
    }
  }

  /// NOT IMPLEMENTED for cloud - Hive is primary source of save
  /// Use syncToCloud instead.
  @override
  ...
  PsychometricProfile _mapSupabaseDataToProfile(Map<String, dynamic> data) {
    ...
    return PsychometricProfile(
      ...
      lastUpdated: data['hive_last_updated'] != null 
          ? DateTime.parse(data['hive_last_updated'].toString())
          : DateTime.now(),
