...
///
/// Phase 63: Now supports dual-write to Hive (local) and Supabase (cloud).
///
...
class PsychometricProvider extends ChangeNotifier {
  ...
  PsychometricProvider(
    this._repository,
    this._engine, {
    SupabasePsychometricRepository? cloudRepository,
    ...
    try {
      ...
      if (cloudProfile != null) {
        ...
        if (cloudTime.isAfter(localTime.add(const Duration(minutes: 1)))) {
          if (kDebugMode) {
            debugPrint(
                'PsychometricProvider: Cloud profile is newer. Updating local.');
          }
  ...
  Future<void> _saveAndSync(PsychometricProfile profile) async {
    ...
    if (_cloudRepository != null) {
      _cloudRepository!.syncToCloud(_profile).then((_) {
        ...
        // Will retry on next save/init logic implicitly as isSynced stays false
      });
    }
  }
  ...
  Future<void> recordCompletion({bool wasRecovery = false}) async {
    final newProfile =
        _engine.onHabitComplete(_profile, wasRecovery: wasRecovery).copyWith(
              isSynced: false,
              lastUpdated: DateTime.now(),
            );
    await _saveAndSync(newProfile);
  ...
  Future<void> clear() async {
    ...
    // Cloud clear is intentionally not called - user likely only wants local clear
    notifyListeners();
  }
  ...
  Future<void> updateFromToolCall(Map<String, dynamic> args) async {
    ...
    final newProfile = _profile.copyWith(
      ...
      antiIdentityLabel:
          args['anti_identity_label'] as String? ?? _profile.antiIdentityLabel,
      antiIdentityContext: args['anti_identity_context'] as String? ??
          _profile.antiIdentityContext,

      // Trait 2: Failure Archetype
      failureArchetype:
          args['failure_archetype'] as String? ?? _profile.failureArchetype,
      failureTriggerContext: args['failure_trigger_context'] as String? ??
          _profile.failureTriggerContext,

      // Trait 3: Resistance Pattern
      resistanceLieLabel:
          args['resistance_lie_label'] as String? ?? _profile.resistanceLieLabel,
      resistanceLieContext: args['resistance_lie_context'] as String? ??
          _profile.resistanceLieContext,
    ...
    if (kDebugMode) {
      debugPrint(
          'PsychometricProvider: Profile saved. Onboarding complete: ${_profile.isOnboardingComplete}');
    }
  }
  ...
  Future<void> updateFromOnboardingData(onboarding.OnboardingData data) async {
    final newProfile = _profile.copyWith(
      ...
      resistanceLieLabel:
          data.resistanceLieLabel ?? _profile.resistanceLieLabel,
      isSynced: false,
      lastUpdated: DateTime.now(),
    );
    await _saveAndSync(newProfile);
  ...
  Future<void> logPermissionRefusal(String scope) async {
    ...
    if (!_profile.declinedPermissions.contains(scope)) {
      final updatedRefusals = List<String>.from(_profile.declinedPermissions)
        ..add(scope);
      final newProfile = _profile.copyWith(
  ...
  Future<bool> finalizeOnboarding() async {
    try {
      ...
      if (kDebugMode) {
        debugPrint('PsychometricProvider: Profile persisted to Hive + Cloud');
  ...
  Future<void> analyzeTranscript(List<Map<String, String>> transcript) async {
    if (transcript.isEmpty) {
      if (kDebugMode) {
        debugPrint('PsychometricProvider: Empty transcript, skipping analysis.');
      }
      return;
    }
    ...
    if (kDebugMode) {
      debugPrint(
          'PsychometricProvider: Analyzing ${transcript.length} turns via DeepSeek V3...');
    }
    ...
    try {
      ...
      buffer.writeln(
          "Analyze this specialized coaching session transcript and extract the user's psychometric profile.");
      buffer.writeln("Return ONLY a JSON object with the following schema:");
      buffer.writeln("{");
      buffer.writeln(
          '  "anti_identity_label": "The specific identity they fear becoming (e.g. The Drifter, The Skeptic)",');
      buffer.writeln(
          '  "anti_identity_context": "Why they fear this (1 sentence)",');
      buffer.writeln(
          '  "failure_archetype": "Their specific failure pattern (e.g. Perfectionism, Procrastination, Overthinking)",');
      buffer.writeln(
          '  "failure_trigger_context": "What triggers this failure (1 sentence)",');
      buffer.writeln(
          '  "resistance_lie_label": "The lie they tell themselves to avoid change (e.g. I\'ll do it tomorrow, I need more research)",');
      buffer.writeln(
          '  "resistance_lie_context": "The deeper truth behind the lie (1 sentence)",');
        ...
        'messages': [
          {
            'role': 'system',
            'content':
                'You are an expert psychometrician. Extract the personality profile from the transcript. Return JSON only.'
          },
      ...
      if (kDebugMode) {
        debugPrint('PsychometricProvider: Sending request to DeepSeek...');
      }
      ...
      if (kDebugMode) {
        debugPrint('PsychometricProvider: Analysis complete. Parsing JSON...');
      }
  ...
  Future<void> syncSensors() async {
    if (kDebugMode) {
      debugPrint('PsychometricProvider: Syncing Sherlock Sensors...');
    }
    ...
    // 4. Persist (local only - sensor data never syncs to cloud per privacy contract)
    await _repository.saveProfile(_profile);
    notifyListeners();
