...
|--------|-------------|-----------|
| **A: 12 Archetypes** | DeepSeek's proposal (Builder, Nurturer, Warrior, etc.) | Good coverage, moderate content burden |
...
**Recommendation:** Option A (12) — validated by Jungian archetype literature, balances coverage with content creation burden.

**Impact:**
- Blocks: F-06 (archetype_templates table), F-13 (habit templates), F-14 (archetype content)
- Affects: Content creation pipeline, facet-to-content mapping accuracy
...
**Recommendation:** Option B — transparency supports autonomy (aligns with psyOS Parliament philosophy).

**Impact:**
- Blocks: F-11 (feedback signal tracking), preference embedding UI design
- Affects: User trust, sense of control, debugging capability
...
| **Status** | 🔴 PENDING |
| **Requires** | None — architectural decision |
| **Generated By** | RQ-005/006/007 Deep Analysis (10 Jan 2026) |

**Context:**
DeepSeek's "Energy Gating" filter removes habits requiring `high_focus` if facet operates during `recovery` hours. But facets don't currently have an energy state association.

**Options:**

| Option | Description | Trade-off |
|--------|-------------|-----------|
| **A: Explicit field** | User configures `typical_energy_state` per facet | Accurate but adds onboarding friction |
| **B: Infer from behavior** | Learn from habit completion times | No friction but takes time to learn |
| **C: Time-of-day proxy** | Derive from facet activation patterns | Middle ground |

**Recommendation:** Option C — least friction, reasonable accuracy based on when facet is typically active.

**Impact:**
- Blocks: Energy Gating implementation, RQ-014 integration
- Affects: Recommendation quality for energy-mismatched habits
...
**Context:**
The Architect generates recommendation cards and places them in JITAI's `content_queue`. The Commander decides WHEN to show them. Cards may sit in queue for days if user is in wrong V-O state.
...
| **C: Context-sensitive** | Expire based on changing user context | Accurate but complex implementation |

**Recommendation:** Option B (7-day TTL) — simple implementation, reasonable freshness guarantee.

**Impact:**
- Blocks: F-10 (Architect scheduler), content_queue schema design
- Affects: Recommendation relevance, compute efficiency
