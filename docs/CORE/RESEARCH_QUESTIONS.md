...
| **Blocking** | Content creation, Facet-to-content mapping |
| **Generated By** | RQ-005/006/007 Deep Analysis (10 Jan 2026) |
| **Assigned** | Deep Think session required |
...
|---|----------|---------------|
| 1 | What is the precise psychological definition of each archetype? | Content tone, habit selection |
| 2 | What is the 768-dim embedding for each archetype name? | Vector similarity matching |
| 3 | What is the 6-dim ideal_dimension_vector for each archetype? | Stage 2 psychometric alignment |
| 4 | How do we validate archetype assignments (user feedback, A/B)? | Accuracy measurement |
| 5 | Should users override automatic archetype mapping? | Autonomy vs simplicity |
| 6 | How do we handle facets that don't cleanly match any archetype? | Edge case UX |

**Proposed 12 Archetypes (Needs Validation):**

| Archetype | Core Focus | Dimension Emphasis |
|-----------|------------|-------------------|
...
**Output Expected:**
- Precise archetype definitions (1-2 paragraph each)
- 768-dim embedding for each archetype name
- 6-dim ideal_dimension_vector for each archetype
- Mapping validation strategy
- Fallback strategy for poor matches
...
| **Blocking** | Two-Stage Retrieval Stage 2, F-13 content creation |
| **Generated By** | RQ-005/006/007 Deep Analysis (10 Jan 2026) |
| **Assigned** | Content strategy session |

**Context:**
Two-Stage Hybrid Retrieval requires every habit to have BOTH a 768-dim semantic embedding (auto-generated) AND a 6-dim ideal_dimension_vector (manually curated). This manual curation is a content creation burden not previously accounted for.

**Sub-Questions:**

| # | Question | Why It Matters |
|---|----------|---------------|
| 1 | Can we auto-derive dimension vectors from habit descriptions (LLM-assisted)? | Curation efficiency |
| 2 | What validation process ensures dimension vectors are accurate? | Quality control |
| 3 | Should dimension vectors be adjustable based on population learning? | Continuous improvement |
| 4 | How do we handle habits that span multiple dimension poles? | Multi-polar habits |
| 5 | What format should curators use to input dimension values? | Workflow design |

**Output Expected:**
- Curation workflow specification
- LLM-assisted derivation prompt (if applicable)
...
**Context:**
DeepSeek proposes a "preference embedding" (768-dim vector) that fine-tunes Stage 1 retrieval based on implicit feedback signals. When user bans a habit, we "subtract a fraction of this habit's vector from the user's preference vector." But the specifics are undefined.

**Sub-Questions:**

| # | Question | Why It Matters |
|---|----------|---------------|
| 1 | What is α (the fraction) for "ban" vector subtraction? | Magnitude of preference shifts |
| 2 | How do we prevent preference drift from stated aspirations? | Alignment maintenance |
| 3 | Should preference embedding be visible/editable by user? | Transparency, autonomy |
| 4 | How often should we re-compute embedding (every signal vs batched)? | Performance, responsiveness |
| 5 | What's the interaction between preference embedding and Trinity Seed? | Cold start consistency |
| 6 | How do we handle embedding reset if user wants fresh start? | User control |

**Output Expected:**
- Update algorithm specification
- α value recommendation with rationale
- Drift prevention mechanism
- User visibility/control recommendation
...
**Context:**
DeepSeek's "Pace Car Protocol" specifies max 1 recommendation/day, only if user has < 5 active habits per facet. These thresholds are stated as facts but lack supporting research or rationale for the specific numbers.

**Sub-Questions:**

| # | Question | Why It Matters |
|---|----------|---------------|
| 1 | Should recommendation frequency adapt to user engagement level? | Power users vs casual |
| 2 | Is 5 habits/facet the right threshold, or should it be personalized? | Individual capacity |
| 3 | Should multi-facet users get 1/day total or 1/day/facet? | Multi-facet UX |
| 4 | What happens when user explicitly requests recommendations? | On-demand vs passive |
| 5 | Should new users get more recommendations initially? | Onboarding velocity |

**Output Expected:**
- Threshold validation (literature or A/B test design)
- Personalization strategy (if recommended)
- On-demand request handling specification
...
| **Blocking** | F-18 (IdentityRoadmapService), metric strategy |
| **Generated By** | RQ-005/006/007 Deep Analysis (10 Jan 2026) |
| **Assigned** | Any agent |

**Context:**
DeepSeek proposes ICS = Σ(Votes × Consistency) / DaysActive. But we already have hexis_score (undefined in docs) and graceful_score (rolling consistency). Creating parallel metrics causes confusion.

**Sub-Questions:**

| # | Question | Why It Matters |
|---|----------|---------------|
| 1 | What does hexis_score currently measure? | Baseline understanding |
| 2 | Should ICS replace hexis_score or coexist? | Metric consolidation |
| 3 | How is "Votes" defined in the ICS formula? | Implementation clarity |
| 4 | How is "Consistency" calculated (streak vs graceful)? | Alignment with graceful_score |
| 5 | Is ICS per-facet, per-roadmap, or per-habit? | Scope clarity |
| 6 | How do we display ICS to users (Seed/Sapling/Oak)? | UI design |

**Output Expected:**
- hexis_score audit results
- Metric consolidation recommendation
- ICS formula refinement
