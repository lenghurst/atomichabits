...
| **Status** | ✅ **COMPLETE** (14 Jan 2026) |
| **Priority** | **HIGH** — Core to conflict detection |
| **Blocking** | Conflict resolution, JITAI integration, dashboard design |
| **Generated By** | CD-015 (psyOS Architecture) |
| **Assigned** | Deep Think external research (with RQ-048a/b, RQ-014) |
| **Decomposed Into** | RQ-048a (Domain Taxonomy), RQ-048b (Cognitive Limits) |
...
**Deep Think Research Output:**

**Topology Model:** Directed Weighted Graph with "Airlock" Pattern
- **Nodes:** Identity facets (`identity_facets` table)
- **Edges:** Relationships (`identity_topology` table)
- **Direction:** Asymmetric (A → B ≠ B → A)
  - Example: "Runner" → "Entrepreneur" = SYNERGISTIC (energy boost)
  - Example: "Entrepreneur" → "Runner" = COMPETITIVE (time scarcity)

**Interaction Types:**
| Type | Friction Range | Meaning | Example |
|---|---|---|---|
| SYNERGISTIC | 0.0 - 0.3 | Source reduces activation energy for Target | "Runner" energizes "Entrepreneur" |
| COMPETITIVE | 0.3 - 0.7 | Source competes for Time/Money with Target | "Founder" vs "Present Father" |
| ANTAGONISTIC | 0.7 - 1.0 | Source values/identity conflict with Target | "Hedonist" vs "Stoic" |
| DEPENDENT | N/A | Target cannot exist without Source | "Marathon Runner" depends on "Athlete" |

**Airlock Pattern (Security):**
- Composite foreign keys enforce tenant isolation
- Physically impossible to link Facet A (User 1) to Facet B (User 2)
- Uses PostgreSQL UNIQUE (id, user_id) constraints
...
1. **Same Domain:** Default to COMPETITIVE (resource overlap)
2. **Complementary Energy:** Default to SYNERGISTIC (Physical + Focus)
3. **Clashing Energy:** Default to COMPETITIVE (Focus + Social, high switching cost)
4. **User Override:** Always allow manual adjustment

**Sub-Questions Answered:**

| # | Question | Answer |
|---|----------|--------|
| 1 | How do we initially populate the topology? | AI inference from domain + energy state, user-overridable |
| 2 | Should topology edges be bidirectional or directed? | **Directed** — A → B ≠ B → A (asymmetric relationships) |
| 3 | How does friction coefficient affect JITAI decisions? | High friction = avoid back-to-back scheduling |
| 4 | What visualizations make topology intuitive to users? | **Deferred to UX research** (not blocking schema) |
| 5 | How do we detect when topology has changed? | **Deferred to Phase C** (adaptive ML) |
| 6 | Should we use a graph database or relational? | **Relational** — PostgreSQL sufficient, no need for Neo4j |

**Output Delivered:**
- ✅ Graph model specification (directed, weighted, 4 interaction types)
- ✅ Airlock Pattern for tenant isolation
- ✅ Auto-inference algorithm (domain + energy state)
- ✅ Complete schema with triggers
- ✅ Reconciliation document: `/docs/CORE/RECONCILIATION_RQ048ab_RQ014_RQ013_SCHEMA_FOUNDATION.md`

**Code References:**
- Schema: `supabase/migrations/XXX_create_identity_topology.sql` (BLOCKED — Phase A)
- Service: `lib/domain/services/topology_inference_engine.dart` (Phase C)
...
| **Generated By** | CD-015 (psyOS Architecture) |
| **Assigned** | Deep Think external research (with RQ-048a/b, RQ-013) |
...
**Energy State Taxonomy (Validated Against CD-015):**
| State | Neurochemistry | Energy Type | Example Facets |
|---|---|---|---|
| `high_focus` | Dopamine/Acetylcholine | Cognitive | "The Coder", "The Writer", "The Strategist" |
| `high_physical` | Adrenaline/Endorphin | Kinetic | "The Athlete", "The Runner", "The Dancer" |
| `social` | Oxytocin/Serotonin | Interpersonal | "The Networker", "The Parent", "The Friend" |
| `recovery` | Parasympathetic | Restorative | "The Meditator", "The Sleeper", "The Leisure" |

**Asymmetric Switching Cost Matrix (Minutes):**
...
**Key Finding:** Switching costs are **ASYMMETRIC**
- Focus → Social (30 min) ≠ Social → Focus (30 min) — Both high due to Attention Residue (Leroy, 2009)
- Physical → Focus (25 min) > Focus → Physical (15 min) — ANS cool-down required
- Recovery → Any (10-20 min) — Inertia effect (activation energy)
...
**Passive Energy Detection Algorithm:**
```python
def detect_energy_state(ctx):
    # 1. BIOMETRIC OVERRIDE (High Confidence - RQ-010a Bio 15%)
    if ctx.heart_rate > 110: return 'high_physical'
    if ctx.is_sleeping: return 'recovery'

    # 2. ACTIVITY CONTEXT (Medium Confidence - RQ-010a Activity 10%)
    if ctx.activity == 'RUNNING': return 'high_physical'
    if ctx.activity == 'IN_VEHICLE': return 'recovery'

    # 3. SOCIAL/CALENDAR (Medium Confidence - RQ-010a Calendar 15%)
...
| # | Question | Answer |
|---|----------|--------|
| 1 | How do we validate energy state categories? | ✅ Matches CD-015 exactly; literature-backed (Leroy 2009, Ophir 2009) |
| 2 | Are switching costs bidirectional or asymmetric? | **Asymmetric** — 4×4 matrix with different costs per direction |
| 3 | How does chronotype affect transitions? | ✅ Modifiers for INTO high_focus based on time + chronotype (RQ-012) |
| 4 | Should we track actual vs assumed energy? | **Passive detection** from Android APIs (RQ-010a/b permission model) |
| 5 | How do we detect energy from passive signals? | ✅ Algorithm delivered (4-tier confidence hierarchy) |
| 6 | What interventions reduce switching costs? | **Deferred to Phase D** (Airlock content generation) |

**Output Delivered:**
- ✅ Validated 4-state energy taxonomy (matches CD-015)
- ✅ Asymmetric 4×4 switching cost matrix
- ✅ Chronotype modifiers (integration with RQ-012)
- ✅ Passive energy detection algorithm (integration with RQ-010a/b)
- ✅ Reconciliation document: `/docs/CORE/RECONCILIATION_RQ048ab_RQ014_RQ013_SCHEMA_FOUNDATION.md`

**Research Citations:**
- Leroy (2009): Attention Residue and task switching
...
**Code References:**
- Schema: `identity_facets.typical_energy_state` field (BLOCKED — Phase A)
- Service: `lib/domain/services/energy_state_detector.dart` (Phase B)
- JITAI: `lib/domain/services/jitai_timing_optimizer.dart` (Phase C)
...
### RQ-048a: Facet Domain Taxonomy Definition

| Field | Value |
|-------|-------|
| **Question** | What domain categories should identity facets be organized into? |
| **Status** | ✅ **COMPLETE** (14 Jan 2026) |
| **Priority** | **CRITICAL** — Foundation for identity schema |
| **Blocking** | identity_facets table, dashboard visualization, facet creation UX |
| **Generated By** | Protocol 14 (RQ Decomposition) — Parent: RQ-013 |
| **Assigned** | Deep Think external research |

**Context:**
Identity facets need categorical organization for:
- Dashboard visualization (grouping, color coding)
- AI understanding of facet relationships
- User mental model ("Parliament seating arrangement")

**Deep Think Research Output:**
- **Initial Proposal:** 6 domains (vocational, somatic, relational, intellectual, creative, spiritual)
- **Protocol 9 Reconciliation:** Reduced to 4 domains for MVP
  - **Merge:** creative → intellectual (both high_focus, self-actualization)
  - **Reject:** spiritual (NICE-TO-HAVE — can use relational or free-text labels)
...
**Research Citations:**
- Hettler (1976): Six Dimensions of Wellness (adapted)
- Cowan (2001): Working memory capacity and chunking
- Hick's Law: Decision time increases logarithmically with number of choices

**Sub-Questions Answered:**
| # | Question | Answer |
|---|----------|--------|
| 1 | How many domains are optimal? | 4 (vocational, somatic, relational, intellectual) |
| 2 | Should facets have multiple domains? | No — one primary domain for clarity |
| 3 | Are domains fixed or user-extensible? | Fixed enum for MVP (extensible post-launch) |
| 4 | How do domains relate to energy states? | Strong defaults (somatic → high_physical) but user-overridable |

**Output Delivered:**
- ✅ Domain taxonomy SQL enum
- ✅ Mapping table (domain → default energy state → example facets)
- ✅ Reconciliation document: `/docs/CORE/RECONCILIATION_RQ048ab_RQ014_RQ013_SCHEMA_FOUNDATION.md`

**Code References:**
- Schema: `supabase/migrations/XXX_create_identity_facets.sql` (BLOCKED — Phase A not started)
- Model: `lib/domain/models/identity_facet.dart` (BLOCKED)
...
| Field | Value |
|-------|-------|
| **Question** | How many identity facets can users effectively manage? |
| **Status** | ✅ **COMPLETE** (14 Jan 2026) |
| **Priority** | **CRITICAL** — Prevents system overwhelm |
| **Blocking** | Facet creation UX, database triggers, onboarding guidance |
| **Generated By** | Protocol 14 (RQ Decomposition) — Parent: RQ-013 |
| **Assigned** | Deep Think external research |

**Context:**
Without limits, users create too many identity facets, leading to:
- Decision paralysis (too many "Selves" negotiating)
- Database clutter
- UI/UX degradation (scrolling Parliament)

**Deep Think Research Output:**
- **Active Limit ("The Cabinet"):** Soft Cap of 5 facets
  - Based on Cowan (2001): Working memory capacity = 4±1 chunks
  - UX warns user when exceeding 5
  - DB safety cap: 9 (prevents abuse without blocking power users)
- **Total Limit:** Hard Cap of 12 facets (MODIFIED from Deep Think's 15)
  - Original proposal: 15 (no empirical basis)
  - Protocol 10 flagged as LOW-validity assumption
  - Modified: 12 = 2× safety cap (sufficient for edge cases)

**Approved Limits:**
| Limit Type | Count | Enforcement | Rationale |
|---|---|---|---|
| **Soft Cap** (UX) | 5 active | Warning dialog | Cowan (2001): Working memory = 4±1 |
| **Safety Cap** (DB) | 9 active | Database trigger | Prevents abuse, allows flexibility |
| **Hard Cap** (DB) | 12 total | Database trigger | 2× safety cap, sufficient for edge cases |
...
- Cowan (2001): Updated capacity to 4±1 for complex items
- Linville (1987): Self-complexity and psychological resilience
- Hick's Law: Decision time increases with number of choices

**Sub-Questions Answered:**
| # | Question | Answer |
|---|----------|--------|
| 1 | What's the active facet limit? | 5 (soft UX), 9 (hard DB) |
| 2 | What's the total facet limit? | 12 (hard cap) |
| 3 | How to enforce limits? | Database trigger + UX warning |
| 4 | Should limits be configurable? | No — fixed for MVP (research-backed) |

**Output Delivered:**
- ✅ Cognitive load analysis
- ✅ Three-tier limit system (5/9/12)
- ✅ Database trigger implementation
- ✅ Reconciliation document: `/docs/CORE/RECONCILIATION_RQ048ab_RQ014_RQ013_SCHEMA_FOUNDATION.md`

**Code References:**
- Schema: `supabase/migrations/XXX_create_facet_limits_trigger.sql` (BLOCKED)
- UX: `lib/presentation/screens/parliament/create_facet_dialog.dart` (BLOCKED)
