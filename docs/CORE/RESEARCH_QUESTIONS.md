...
| **Status** | 🔴 NEEDS RESEARCH |
| **Priority** | HIGH — Foundational for governance quality |
| **Blocking** | IMPLEMENTATION_PROMPT_GUIDANCE.md creation |
| **Created** | 11 January 2026 (Session 22 — Governance Gap Analysis) |

**Context:**
- "Implementation Prompts" are a new artifact type identified during Session 22
- Research Prompts target RQs (external thinking); Implementation Prompts target Tasks (execution)
- Current governance has no formal definition or quality criteria for Implementation Prompts
- Phase A Schema Foundation prompt was created ad-hoc without standards

**Why This Research Matters:**
Without standardized Implementation Prompt structure:
1. Agents may produce inconsistent outputs
2. Prerequisites may not be verified
3. Constraints may be missed
4. Output format may be ambiguous
5. Verification criteria may be undefined

**Sub-Questions:**
1. What structural patterns work best for Gemini (XML tags, Markdown, hybrid)?
2. How should prerequisites be embedded and verified (Gate Check pattern)?
3. What level of specification prevents ambiguity without over-constraining?
4. How should verification criteria be expressed for self-checking?
5. What role should examples play (few-shot patterns)?
6. How should Implementation Prompts differ from Research Prompts?
7. What's the optimal prompt length for implementation tasks (token efficiency)?

**Expected Outputs:**
1. `IMPLEMENTATION_PROMPT_GUIDANCE.md` — Quality standards and templates
2. Naming convention for Implementation Prompts (`IMPLEMENTATION_PROMPT_*.md`)
3. Gate Check template (Protocol 13 integration)
4. Quality scoring rubric (like Research Prompt 10-point scale)
...
- Internal: `DEEP_THINK_PROMPT_GUIDANCE.md` — existing pattern for Research Prompts

**Related Artifacts:**
- Protocol 13 (Prerequisites Gate Check) — integration point
- `DEEP_THINK_PROMPT_PHASE_A_SCHEMA_FOUNDATION.md` — first Implementation Prompt (ad-hoc)
- Governance Gap Analysis: `docs/analysis/GOVERNANCE_GAP_ANALYSIS_IMPLEMENTATION_PROMPTS.md`
