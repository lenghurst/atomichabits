...
> **Last Updated:** 05 January 2026 (Added RQ-012 through RQ-018 for psyOS, CD-015 confirmed)
> **Purpose:** Track active research informing product/architecture decisions
> **Owner:** Oliver (with AI agent research support)
> **Status:** CD-015 (psyOS) CONFIRMED — New research questions generated
...
CREATE TABLE psychometric_roots (
  user_id UUID PRIMARY KEY,
  root_fear TEXT,                    -- "I am unworthy of love"
  base_temperament_vector JSONB,     -- Biological baseline
  chronotype TEXT                    -- "Wolf", "Lion", "Bear", "Dolphin"
);
...
CREATE TABLE psychological_manifestations (
  ...
  resistance_script TEXT,            -- "If I say no, I will be fired"
  coaching_strategy TEXT             -- "Compassionate Inquiry"
);
```
...
| # | Question | Implications |
|---|----------|--------------|
| 1 | How does Sherlock extract the Root Fear vs Surface Manifestation? | Prompt engineering |
| 2 | Should users ever see their Root Fear, or only manifestations? | UX/therapeutic ethics |
| 3 | How do we link manifestations back to roots for coaching? | AI reasoning |
| 4 | What happens if users have multiple Root Fears? | Schema complexity |
| 5 | How does chronotype (Wolf/Lion/Bear/Dolphin) affect JITAI? | Timing intelligence |

**Output Expected:**
- Schema design for `psychometric_roots` and `psychological_manifestations`
- Sherlock extraction prompts for root vs manifestation
- Coaching logic for cross-facet pattern recognition
- UX guidelines for surfacing (or hiding) root psychology
...
| # | Question | Implications |
|---|----------|--------------|
| 1 | How do we construct AI "agents" representing each facet? | Prompt engineering |
| 2 | Should facet agents have distinct "voices" (tone, vocabulary)? | Character design |
| 3 | How do we prevent the council from feeling gimmicky? | UX quality |
| 4 | What conflict types trigger Council vs normal coaching? | Activation rules |
| 5 | How do we implement "Treaty" proposals and user voting? | Interaction flow |
| 6 | Can Council run asynchronously (notification-based)? | Async UX |
| 7 | What happens when Council can't reach consensus? | Edge case handling |

**Output Expected:**
- Prompt architecture for multi-agent simulation
- Character specifications per facet archetype
- UX flow for Council sessions (sync and async)
- Treaty proposal and resolution mechanics
- Integration with voice and text interfaces
...
PENDING RESEARCH (psyOS - CRITICAL):
RQ-012 (Fractal Trinity Architecture) 🔴 NEEDS RESEARCH
    └── Blocks: Schema, Sherlock, coaching logic
...
RQ-016 (Council AI) 🔴 NEEDS RESEARCH
    └── Blocks: AI prompts, conflict resolution
...
1. **CRITICAL:** RQ-012, RQ-016 (Fractal Trinity, Council AI — signature features)
